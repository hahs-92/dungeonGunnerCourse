    ...
    private IEnumerator MoveEnemyRoutine(Stack<Vector3> movementSteps)
    {
        while (movementSteps.Count > 0)
        {
            ...
            while (Vector3.Distance(nextPosition, transform.position) > 0.2f)
            {
                ...
                enemy.movementToPositionEvent
                    .CallMovementToPositionEvent(nextPosition, transform.position, moveSpeed, (nextPosition - transform.position).normalized);
            ...
            for (int l = 0; l < 8; l++)
            {
                ...
                try
                {
                    obstacle = Mathf.Min(
                        ...
                        surroundingPositionList[index].x, adjustedPlayerCellPositon.y + 
                        surroundingPositionList[index].y],
                    ...
                    if (obstacle != 0)
                    {
                        return new Vector3Int(
                            playerCellPosition.x + 
                            surroundingPositionList[index].x, playerCellPosition.y + 

...
public static class AStar
{
    ...
    public static Stack<Vector3> BuildPath(Room room, Vector3Int startGridPosition, Vector3Int endGridPosition)
    {
        ...
        List<Node> openNodeList = new List<Node>(); // list of nodes / squares to be evaluated
        HashSet<Node> closedNodeHashSet = new HashSet<Node>(); // list of nodes / squares already evaluated
        ...
        Node endPathNode = FindShortestPath(
            startNode, 
            targetNode,
            gridNodes, 
            openNodeList, 
    ...
    /// </summary>
    private static Node FindShortestPath(
        Node startNode, 
        Node targetNode, 
        GridNodes gridNodes, 
    ...
    private static Stack<Vector3> CreatePathStack(Node targetNode, Room room)
    {
        ...
        // multiplicamos x 5, para obtener la mitad de la casilla
        ...
        while (nextNode != null)
        {
            ...
            Vector3 worldPosition = room.instantiatedRoom.grid.CellToWorld(
                new Vector3Int(nextNode.gridPosition.x +
                room.templateLowerBounds.x, nextNode.gridPosition.y +
                room.templateLowerBounds.y, 0));
    ...
    /// </summary>
    private static void EvaluateCurrentNodeNeighbours(
        Node currentNode, 
        Node targetNode, 
        GridNodes gridNodes, 
        List<Node> openNodeList, 
    ...
    /// </summary>
    private static Node GetValidNodeNeighbour(
        int neighbourNodeXPosition,
        int neighbourNodeYPosition, 
        GridNodes gridNodes, 
        HashSet<Node> closedNodeHashSet, 
        InstantiatedRoom instantiatedRoom
    )
    {
        ...
        if (
            neighbourNodeXPosition >= instantiatedRoom.room.templateUpperBounds.x - instantiatedRoom.room.templateLowerBounds.x ||
